...
const AboutSection = () => {
  ...
  return (
    ...
    >
      {/* Background elements */}
      <div className="absolute top-40 right-0 w-96 h-96 bg-cyan-500/10 rounded-full filter blur-3xl" />
      <div className="absolute bottom-40 left-0 w-96 h-96 bg-teal-500/10 rounded-full filter blur-3xl" />
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          >
            ...
            {aboutHighlights.map((highlight, index) => (
              <div
                key={index}
                className="group relative bg-gradient-to-br from-gray-900/50 to-black/50 backdrop-blur-xl border border-cyan-500/20 rounded-xl p-6 hover:border-cyan-500/50 transition-all duration-300 hover:transform hover:scale-105"
                style={{ transitionDelay: `${(index + 2) * 100}ms` }}
              >
                {/* Year badge */}
                <div className="absolute -top-3 -right-3 px-3 py-1 bg-gradient-to-r from-cyan-500 to-teal-500 text-white text-xs font-bold rounded-full shadow-lg">
                  {highlight.year}
                </div>
                ...
                {/* Decorative corner */}
                <div className="absolute bottom-2 right-2 w-6 h-6 border-b-2 border-r-2 border-cyan-500/20 group-hover:border-cyan-500/50 transition-colors duration-300" />
              </div>
            ))}
          </div>
        </div>
